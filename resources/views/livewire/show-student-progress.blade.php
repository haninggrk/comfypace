    ...
    <table class="min-w-full divide-y divide-gray-200">
        ...
        <tbody>
            @foreach ($studentList as $student)
                <tr class="bg-white">
                    <td class="px-6 py-4 whitespace-nowrap text-sm text-left font-medium text-gray-900">{{ $student->name }}</td>
                    <td class="px-6 py-4 whitespace-nowrap text-sm text-center text-gray-500">
                        {{ $student->studentDetail->point }}</td>
                        @php($progress = $student->Progresses->
                            whereIn('milestone_id',$milestone->
                            whereIn('project_id',$class->course->projects->sortBy('order_number')->pluck('id'))->pluck('id'))->last())
                    <td class="px-6 py-4 whitespace-nowrap text-sm text-center text-gray-500">{{ $progress->milestone->project->project_title ?? "Not Found" }}</td>
