        ...
        <li class="flex">
          ...
          <path d="M.293 0l22 22-22 22h1.414l22-22-22-22H.293z" />
           ...
           @endphp
            ...
            @elseif(Auth::use  @if(Auth::user()->role == 2)
            @php
            $classRn = Auth::user()->ClassMembers->where('class_id',$milestone->project->course->classes->pluck('id')->intersect(Auth::user()->ClassMembers->pluck('class_id'))->first())->first()->class
           @endphpr()->role == 1 and Auth::user()->EmployeeDetail->role->position=="Teacher")
          ...
          <div class="bg-white shadow overflow-hidden sm:rounded-lg mb-5">
            <div class="px-4 py-5 sm:px-6">
              <div class="flex justify-between items-center">
                <div>
              <h3 class="text-lg leading-6 font-medium text-gray-900">{{$studentProject->submission_title}}</h3>
              <p class="mt-1 max-w-2xl text-sm text-gray-500">Source: {{$studentProject->submission_url??'-'}}</p>
         
            </div>
         
        </div>
        <div class="video-container mt-3 overflow-hidden relative">
          <div class="absolute w-full" style="height:60px;"></div>
          <iframe id="iframesoal"  class="video" style="" src="{{$studentProject->submission_url}}embed" sandbox="allow-forms allow-scripts allow-pointer-lock allow-same-origin allow-top-navigation" allowfullscreen></iframe>
